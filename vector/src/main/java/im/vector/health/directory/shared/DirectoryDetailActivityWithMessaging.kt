...
abstract class DirectoryDetailActivityWithMessaging: MXCActionBarActivity(), MatrixChatActionHandler {
    ...
    internal fun startCall(isVideo: Boolean) {
        ...
        mSession.mCallsManager.createCallInRoom(mCallableRoom?.roomId, isVideo, object : ApiCallback<IMXCall?> {
            override fun onSuccess(call: IMXCall?) {
                call?.let { mxCall ->
                    UiThreadUtil.runOnUiThread {
                        val intent = Intent(this@DirectoryDetailActivityWithMessaging, VectorCallViewActivity::class.java)
            ...
            override fun onNetworkError(e: Exception) {
                Toast.makeText(this@DirectoryDetailActivityWithMessaging, e.localizedMessage, Toast.LENGTH_SHORT).show()
                ...
                if (e is MXCryptoError) {
                    if (MXCryptoError.UNKNOWN_DEVICES_CODE == e.errcode) {
                        CommonActivityUtils.displayUnknownDevicesDialog(mSession,
                                this@DirectoryDetailActivityWithMessaging,
    ...
    internal val mCreateDirectMessageCallBack: ApiCallback<String> = object : ApiCallback<String> {
        override fun onSuccess(roomId: String) {
            ...
            CommonActivityUtils.goToRoomPage(this@DirectoryDetailActivityWithMessaging, mSession, params)
        }
